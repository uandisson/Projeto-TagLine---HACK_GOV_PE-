...
class NotificationSummaryManager {
    ...
    static void updatePossibleDependentSummaryOnDismiss(Context context, SQLiteDatabase sQLiteDatabase, int androidNotificationId) {
        StringBuilder sb;
        Context context2 = context;
        SQLiteDatabase writableDb = sQLiteDatabase;
        String[] strArr = {OneSignalDbContract.NotificationTable.COLUMN_NAME_GROUP_ID};
        new StringBuilder();
        Cursor cursor = writableDb.query(OneSignalDbContract.NotificationTable.TABLE_NAME, strArr, sb.append("android_notification_id = ").append(androidNotificationId).toString(), (String[]) null, (String) null, (String) null, (String) null);
        if (cursor.moveToFirst()) {
            String group = cursor.getString(cursor.getColumnIndex(OneSignalDbContract.NotificationTable.COLUMN_NAME_GROUP_ID));
            cursor.close();
            if (group != null) {
                updateSummaryNotificationAfterChildRemoved(context2, writableDb, group, true);
                return;
            }
            return;
        }
    ...
    static void updateSummaryNotificationAfterChildRemoved(Context context, SQLiteDatabase writableDb, String group, boolean dismissed) {
        try {
            Cursor cursor = internalUpdateSummaryNotificationAfterChildRemoved(context, writableDb, group, dismissed);
    ...
    private static Cursor internalUpdateSummaryNotificationAfterChildRemoved(Context context, SQLiteDatabase sQLiteDatabase, String str, boolean z) {
        ...
        Cursor cursor = writableDb.query(OneSignalDbContract.NotificationTable.TABLE_NAME, strArr2, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", new String[]{group}, (String) null, (String) null, "_id DESC");
        ...
        if (notifsInGroup == 0) {
            ...
            new StringBuilder();
            int update = writableDb.update(OneSignalDbContract.NotificationTable.TABLE_NAME, values, sb.append("android_notification_id = ").append(androidNotifId).toString(), (String[]) null);
            return cursor;
    ...
    private static void restoreSummary(Context context, String group) {
        ...
        Cursor cursor = null;
        try {
            cursor = OneSignalDbHelper.getInstance(context2).getReadableDbWithRetries().query(OneSignalDbContract.NotificationTable.TABLE_NAME, NotificationRestorer.COLUMNS_FOR_RESTORE, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", new String[]{group}, (String) null, (String) null, (String) null);
    ...
    static Integer getSummaryNotificationId(SQLiteDatabase writableDb, String str) {
        StringBuilder sb;
        String group = str;
        Integer androidNotifId = null;
        Cursor cursor = null;
        try {
            cursor = writableDb.query(OneSignalDbContract.NotificationTable.TABLE_NAME, new String[]{OneSignalDbContract.NotificationTable.COLUMN_NAME_ANDROID_NOTIFICATION_ID}, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 1", new String[]{group}, (String) null, (String) null, (String) null);
            if (!cursor.moveToFirst()) {
                cursor.close();
                if (cursor != null && !cursor.isClosed()) {
                    cursor.close();
    ...
    static void clearNotificationOnSummaryClick(Context context, SQLiteDatabase sQLiteDatabase, String str) {
        ...
        String group = str;
        Integer groupId = getSummaryNotificationId(db, group);
