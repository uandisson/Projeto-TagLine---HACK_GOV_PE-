...
class BadgeCountUpdater {
    ...
    static void update(SQLiteDatabase sQLiteDatabase, Context context) {
        SQLiteDatabase readableDb = sQLiteDatabase;
        Context context2 = context;
        if (areBadgesEnabled(context2)) {
            if (Build.VERSION.SDK_INT >= 23) {
                updateStandard(context2);
            } else {
                updateFallback(readableDb, context2);
            }
        }
    }
    ...
    private static void updateFallback(SQLiteDatabase readableDb, Context context) {
        Cursor cursor = readableDb.query(OneSignalDbContract.NotificationTable.TABLE_NAME, (String[]) null, OneSignalDbHelper.recentUninteractedWithNotificationsWhere().toString(), (String[]) null, (String) null, (String) null, (String) null, NotificationLimitManager.MAX_NUMBER_OF_NOTIFICATIONS_STR);
