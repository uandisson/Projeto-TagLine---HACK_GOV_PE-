...
class NotificationRestorer {
    ...
    private static void queryAndRestoreNotificationsAndBadgeCount(Context context, OneSignalDbHelper dbHelper, StringBuilder sb) {
        StringBuilder sb2;
        Context context2 = context;
        ...
        try {
            ...
            cursor = readableDb.query(OneSignalDbContract.NotificationTable.TABLE_NAME, COLUMNS_FOR_RESTORE, dbQuerySelection.toString(), (String[]) null, (String) null, (String) null, "_id DESC", NotificationLimitManager.MAX_NUMBER_OF_NOTIFICATIONS_STR);
            showNotificationsFromCursor(context2, cursor, 200);
            BadgeCountUpdater.update(readableDb, context2);
    ...
    private static Intent addRestoreExtras(Intent intent, Cursor cursor) {
        ...
        Intent putExtra = intent2.putExtra("json_payload", fullData).putExtra("android_notif_id", existingId).putExtra("restoring", true).putExtra(OneSignalDbContract.OutcomeEventsTable.COLUMN_NAME_TIMESTAMP, Long.valueOf(cursor2.getLong(cursor2.getColumnIndex(OneSignalDbContract.NotificationTable.COLUMN_NAME_CREATED_TIME))));
        return intent2;
    }
    ...
    static void startDelayedRestoreTaskFromReceiver(Context context) {
        ...
        PendingIntent pendingIntent = PendingIntent.getService(context2, RESTORE_KICKOFF_REQUEST_CODE, intentForService, Declaration.PACKAGE_ACCESS);
        ((AlarmManager) context2.getSystemService(NotificationCompat.CATEGORY_ALARM)).set(1, System.currentTimeMillis() + 15000, pendingIntent);
    }
}
