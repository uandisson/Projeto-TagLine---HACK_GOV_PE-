...
class PathInterpolatorDonut implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f391mX;

    /* renamed from: mY */
    private final float[] f392mY;
    ...
    public PathInterpolatorDonut(Path path) {
        ...
        this.f391mX = new float[numPoints];
        this.f392mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            boolean posTan = pathMeasure2.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
            this.f391mX[i] = position[0];
            this.f392mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int startIndex = 0;
        int endIndex = this.f391mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f391mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f391mX[endIndex] - this.f391mX[startIndex];
        if (xRange == 0.0f) {
            return this.f392mY[startIndex];
        }
        float fraction = (t - this.f391mX[startIndex]) / xRange;
        float startY = this.f392mY[startIndex];
        return startY + (fraction * (this.f392mY[endIndex] - startY));
    }
