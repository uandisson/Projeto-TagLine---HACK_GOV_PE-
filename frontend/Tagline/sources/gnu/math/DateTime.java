...
public class DateTime extends Quantity implements Cloneable {
    ...
    public int parseDate(String str, int i, int i2) {
        ...
        if (day > 0 && pos3 == start3 + 2) {
            if ((mask2 & 4) == 0) {
                ...
                maxDay = daysInMonth(month - 1, (mask2 & 2) != 0 ? year : MapViewConstants.ANIMATION_DURATION_LONG);
            }
            if (day <= maxDay) {
    ...
    public static boolean isLeapYear(int i) {
        int year = i;
        return year % 4 == 0 && (year % 100 != 0 || year % SVG.Style.FONT_WEIGHT_NORMAL == 0);
    }
    ...
    public static int compare(DateTime dateTime, DateTime dateTime2) {
        ...
        if (((date1.mask | date2.mask) & 14) == 0) {
            if (millis1 < 0) {
                millis1 += OpenStreetMapTileProviderConstants.ONE_DAY;
            }
            if (millis2 < 0) {
                millis2 += OpenStreetMapTileProviderConstants.ONE_DAY;
            }
        }
    ...
    private static void append(int value, StringBuffer stringBuffer, int minWidth) {
        ...
        while (true) {
            padding--;
            if (padding >= 0) {
                StringBuffer insert = sbuf.insert(start, Dimension.SYM_P);
            } else {
                return;
            }
        }
    ...
    public void toString(StringBuffer stringBuffer) {
        ...
        if (hasDate) {
            toStringDate(sbuf);
            if (hasTime) {
                StringBuffer append = sbuf.append(Dimension.SYM_TRUE);
            }
        }
        if (hasTime) {
            toStringTime(sbuf);
