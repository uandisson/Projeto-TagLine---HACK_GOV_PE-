    ...
    public void applyTransformation(float f, Ring ring, boolean z) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                endTrim = startTrim + (0.79f * MATERIAL_INTERPOLATOR.getInterpolation(interpolatedTime / 0.5f)) + MIN_PROGRESS_ARC;
            } else {
                endTrim = ring2.getStartingStartTrim() + 0.79f;
                startTrim = endTrim - ((0.79f * (1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - 0.5f) / 0.5f))) + MIN_PROGRESS_ARC);
            }
